...
int updateMedicineService(MedicineService* service, int medicineId, char* name, int concentration) {
    ...
    if(medicine != NULL) {
        ...
        // if valid, update
        updateMedicineRepository(medicine, name, concentration);
...
static int compareByQuantityAscending(const void *item1, const void *item2) {
...
static int compareByQuantityDescending(const void *item1, const void *item2) {
    Medicine* medicine1 = (Medicine*)item1;
    Medicine* medicine2 = (Medicine*) item2;
...
static int compareByNameAscending(const void *item1, const void *item2) {
    Medicine* medicine1 = (Medicine*)item1;
    Medicine* medicine2 = (Medicine*) item2;
...
static int compareByNameDescending(const void *item1, const void *item2) {
    Medicine* medicine1 = (Medicine*)item1;
    Medicine* medicine2 = (Medicine*) item2;
...
List* getAllMedicineSortedByQuantityService(MedicineService* service, int type) {
    ...
    // 0 - ascending
    if(type == ASC) {
		selectionSort(medicineList, compareByQuantityAscending);
    // 1 - descending
    } else {
		selectionSort(medicineList, compareByQuantityDescending);
    }
...
List* getAllMedicineSortedByNameService(MedicineService* service, int type) {
    ...
    // 0 - ascending
    if(type == ASC) {
		selectionSort(medicineList, compareByNameAscending);
        // 1 - descending
    } else {
		selectionSort(medicineList, compareByNameDescending);
    }
...
List* getAllMedicineFiltredByNameService(MedicineService* service, char letter) {
	List* medicineList = getAllMedicineRepository(service->repository);
	List* filteredList = createList(destroyMedicine, getSizeOfMedicinePointer());
    Medicine* medicine;
